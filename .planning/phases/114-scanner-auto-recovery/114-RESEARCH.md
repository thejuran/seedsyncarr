...
| `_is_transient_ssh_error` / `_is_permanent_ssh_error` | `remote_scanner.py:18-28` | Substring matchers | Reuse; add a sibling `_is_name_resolution_ssh_error` |
...
| New helper + in-loop transient handling | Handle name-resolution purely in the new retry loop (no `sshcp.py` change) | Workable, but scatters the classification away from the centralized tuples. The new-tuple approach keeps classification co-located and self-documenting. Marginal preference for the tuple. |
...
```
 ...
 (ScannerProcess         │     try: out = ssh.shell(scanfs ...)  ──► success ─────────┼──► (files, total, used)
  .run_loop)             │     except SshcpError as e:                                │        ▲ recovered blip
                         │        if permanent (not name-res):  raise recoverable=F ──┼──┐     never reaches
                         │        if SystemScannerError:        raise recoverable=F ──┼──┤     propagation
                         │        if transient OR name-res:                           │  │
                         │            if attempt < MAX:  sleep(backoff w/ jitter,     │  │
                         │                                honor terminate Event); ────┼──┘ (loop)
                         │                                continue                    │
...
A reader can trace the DNS-blip use case: blip enters at `ssh.shell()`, the retry loop sleeps and retries, the host resolves, `scan()` returns files — propagation is never reached. A genuinely-wrong host: loop exhausts → `recoverable=False` → propagation → controller dies → first death triggers `ServiceRestart` → re-init scan also exhausts → eventually cap reached → today's `server.up=False` surface.
...
**What:** Wrap the SSH scan call in `RemoteScanner.scan()` in a bounded `for` loop with backoff. Retry on `_is_transient_ssh_error` OR `_is_name_resolution_ssh_error`; never retry `SystemScannerError` or other permanent errors.
...
class RemoteScanner(IScanner):
    ...
    __SCAN_BACKOFF_CEILING_SECS = 8.0
    __SCAN_BACKOFF_JITTER = 0.2      # ±20%
```
```python
# scan() retry shape (replaces remote_scanner.py:88-111). Classification helper
# keeps SCAN-03 byte-for-byte identical on exhaustion.
...
for attempt in range(1, self.__SCAN_MAX_ATTEMPTS + 1):
    try:
        ...
        retryable = (self._is_transient_ssh_error(e)
                     or self._is_name_resolution_ssh_error(e))
        # SystemScannerError and genuine permanent errors are NEVER retried.
        if "SystemScannerError" in str(e) or not retryable:
        ...
        if attempt < self.__SCAN_MAX_ATTEMPTS:
            ...
            continue
        # Exhausted: surface exactly as today (SCAN-03).
        raise ScannerError(
            Localization.Error.REMOTE_SERVER_SCAN.format(str(e).strip()),
            recoverable=False)
```
The `__to_scanner_error` helper preserves the EXACT existing recoverable rules from `remote_scanner.py:95-111` for the non-retried (immediate raise) branch — including the `__first_run` interaction (see Pitfall 4). Factor it out so the loop body stays readable.
...
- **Per-attempt ceiling 8s**, so even at max the total added latency for a genuinely-wrong host is ≤ ~3s of sleeps (1s + 2s), well within the 60s pytest-timeout and well under the 30s `interval_ms_remote_scan` so a recovered blip stays inside one scan cycle's "feel." [VERIFIED: `seedsyncarr.py:336` `interval_ms_remote_scan = 30000`]
- A DNS blip like the 2026-06-19 incident (host resolved ~2 min later per the debug session) will NOT recover within a 3-attempt/~7s window — and that is correct: it exhausts, surfaces, and RECOV-01's controller auto-restart (with its own re-init scan attempts and longer cadence) becomes the recovery mechanism for multi-minute outages. The two bounds compose: seconds-scale in-scan retry for sub-10s blips, restart-scale recovery for minutes-scale outages. [CITED: debug/resolved/seedbox-files-not-showing.md — "host resolved fine ~2 min later"]
...
- **Reset semantics:** D-03 says "a run that stays up past a threshold resets the counter." The reset check belongs in the `except ServiceRestart` branch comparing the prior run's `last_start` against `__RESTART_RESET_SECS`. This makes intermittent failures (one death every few hours) recover indefinitely, while a tight restart loop (deaths within 5 min of each restart) hits the cap quickly.
- **Falls through to today's behavior, not a crash:** once `restart_budget` is False, the code path is byte-for-byte the current `:186-188`. No new crash surface. [VERIFIED]
...
- **Bare `time.sleep(backoff)` in the scan loop:** blocks shutdown responsiveness; the scanner process can't honor its terminate Event during the sleep. Use the Event-aware wait (Pitfall 1).
...
**Key insight:** Nearly everything Phase 114 needs already exists. The genuinely new code is ~one bounded loop in `scan()`, one pattern tuple + matcher, and one counter/reset block in `main()`/`run()`. Resist inventing new mechanisms — the CONTEXT and STATE both stress "wire together infrastructure already present."
...
### Pitfall 2: Breaking the existing `__first_run` strictness semantics
**What goes wrong:** The retry loop and `__first_run` gate interact. Today, before the first successful scan, a non-transient error is fatal (`remote_scanner.py:106-107`) so the user is prompted to fix config; after first success, transient errors are recoverable across intervals. If the new loop blindly retries name-resolution on first run, that is actually DESIRED per D-01 (retry-then-surface applies regardless of first run). But the loop must NOT change first-run behavior for OTHER permanent errors (bad password, host-key change) — those must still surface fatal immediately, even on attempt 1.
**Why it happens:** Easy to over-generalize the retry condition.
**How to avoid:** The retry condition is strictly `_is_transient_ssh_error OR _is_name_resolution_ssh_error`. `Incorrect password` and `Remote host key has changed` are in `PERMANENT_ERROR_PATTERNS` but NOT in `NAME_RESOLUTION_ERROR_PATTERNS`, so they are not retryable and raise immediately with the existing `__first_run`-aware recoverable value. Confirm with the existing tests `test_raises_nonrecoverable_error_on_wrong_password_after_first_run` and `test_raises_nonrecoverable_error_on_host_key_change_after_first_run` (`test_remote_scanner.py:516-590`) — they must still pass unchanged.
**Warning signs:** Those two existing tests fail; or a bad-password scenario takes 3× backoff to surface.
...
**How to avoid:** Wrap only the `self.__ssh.shell(scanfs ...)` call at `:89-92`. Leave install and `df` untouched. Re-run the existing call-count tests; if the retry loop adds retries, those counts change ONLY in the failure path, which existing tests already encode (the loop must not re-run install/df on a main-scan retry).
...
**What goes wrong:** When the loop hits a non-retryable error (permanent, or `SystemScannerError`), it must raise with the SAME recoverable value the current code computes — which depends on `__first_run` and the error class. If you hardcode `recoverable=False` for all immediate raises, you'd change behavior for the first-run-transient edge that the existing logic handles.
**How to avoid:** Factor the existing classification (`remote_scanner.py:95-111`) into a helper `__to_scanner_error(e)` that returns the correctly-flagged `ScannerError`, and call it for the immediate-raise branch. The loop's exhaustion branch raises `recoverable=False` (a blip that never cleared IS fatal after the bounded window). Keep these two raise sites distinct.
...
| A4 | Pitfall-1 option (a) (short sleep, accept worst-case force-terminate) is acceptable vs option (b) (Event-aware wait) | Pitfall 1 | Option (a) can trip the 1s terminate poll → SIGTERM on shutdown-during-backoff. Functionally safe (force-terminate works) but noisy. Planner must choose explicitly. Medium-ish operational risk if (a) chosen with a high ceiling. |

## Open Questions

1. **Backoff shutdown-responsiveness: option (a) vs (b)?**
   ...
   - What's unclear: Whether the team prefers the simpler short-sleep (a) with an accepted force-terminate tradeoff, or the slightly-more-wiring Event-aware wait (b).
   - Recommendation: Default to (b) (pass the terminate Event into `RemoteScanner`, wait on it in `run_init`-wired code) for correctness; it's spawn-safe and adds one clean test. If the planner wants minimal change, (a) with a ≤2s ceiling is acceptable with a documented comment. State the choice in the plan.

2. **Where should the restart cap/reset constants live — `main()` module constants or `Constants`?**
   - What we know: `main()` is the only scope that survives `ServiceRestart`. Scanner constants belong on `RemoteScanner`.
   - What's unclear: Whether the restart constants read better as module-level constants in `seedsyncarr.py` or in `common/constants.py` `Constants`.
   - Recommendation: Module-level constants in `seedsyncarr.py` next to `main()` (the restart logic is local to that file). Use `Constants` only if a value is referenced from more than one module (none are).
...
| SCAN-03 | Existing permanent-error tests (wrong password, host-key change, SystemScannerError, mangled output) still surface `recoverable=False` immediately | unit (regression) | `pytest tests/unittests/test_controller/test_scan/test_remote_scanner.py -x` | ✅ (must stay green) |
...
| RECOV-01 | A run that stays up past the reset threshold resets the consecutive counter | unit | `pytest tests/unittests/test_seedsyncarr.py -k restart_reset -x` | ❌ Wave 0 |
...
### Wave 0 Gaps
- [ ] `tests/unittests/test_controller/test_scan/test_remote_scanner.py` — extend with retry-recovers / retry-exhausts / retry-bounded / name-resolution tests (covers SCAN-01/02/03). Patch `controller.scan.remote_scanner.time.sleep`. Reuse the existing call-counter `ssh_shell` harness (`:631-669`).
- [ ] `tests/unittests/test_seedsyncarr.py` — add restart-cap / cap-exhausted / stayed-up-reset tests (covers RECOV-01). The current file tests static methods only; testing `main()`/`run()` restart bookkeeping needs either a refactor of the counter into a testable pure helper (recommended) or patching. **Planner note:** extracting the restart-budget decision into a small static/pure function (e.g. `_should_auto_restart(consecutive, cap, last_start, reset_secs)`) makes RECOV-01 unit-testable without spinning the full `run()` loop, mirroring how `_emit_startup_warnings`/`_detect_incomplete_config` are tested as static methods.
...
| Denial of recovery via retry storm against the seedbox | Denial of Service | Bounded attempts (SCAN-02) + bounded restarts (RECOV-01) + jittered backoff prevent hammering the remote host. This is the core design. |
...
- Test suite read this session: `tests/unittests/test_controller/test_scan/test_remote_scanner.py`, `test_scanner_process.py`, `test_controller_job.py`, `tests/unittests/test_ssh/test_sshcp.py`, `tests/unittests/test_seedsyncarr.py`.
...
- Pitfalls: HIGH — each pitfall derived from actual code behavior (terminate poll timing, call-count assertions, `__first_run` gate, ruff separate gate, spawn `__getstate__`).
