...
- **D-01 — Retry-then-surface.** Treat all name-resolution failures (`Bad hostname:` / `Could not resolve hostname` / `Name or service not known`) as **transient first** and retry with bounded backoff. The bounded retry **is** the test that distinguishes a blip from a real misconfiguration — no up-front heuristic guessing. On exhaustion, surface exactly as today. Satisfies SCAN-01 + SCAN-02 + SCAN-03 together. Consequence if wrong: a real config typo takes N×backoff longer to surface (bounded seconds) — acceptable.
...
├── controller/scan/remote_scanner.py     # ADD _is_name_resolution_ssh_error + ONE shared bounded retry helper used by scan()'s main call AND _install_scanfs()'s md5sum/copy ops
...
└── tests/unittests/
    ...
    ├── test_controller/test_scan/test_remote_scanner.py   # ADD main-scan + install-path retry-recovers / retry-exhausts / bounded / timeout-unchanged tests
...
def _is_name_resolution_ssh_error(error: SshcpError) -> bool:
    ...
    both the scan and install paths, then surface fatal on exhaustion. See Phase 114 D-01."""
    msg = str(error)
...
```
```python
# ONE shared helper. The retry gate is name-resolution ONLY. On a non-retryable
# error OR name-resolution exhaustion the helper RE-RAISES the original SshcpError
# so each caller's own except converts it on its own path (main scan →
# __to_scanner_error / REMOTE_SERVER_SCAN; install → REMOTE_SERVER_INSTALL). The
# helper itself NEVER builds a ScannerError, so each path keeps its own localized
...
try:
    ...
    if self._is_name_resolution_ssh_error(e):          # exhausted name-resolution
        raise ScannerError(
...
# → recoverable=False for "Bad hostname:" (unchanged), recoverable=True for "Timed out" (unchanged).
```
...
**What:** On exhaustion, the main-scan path raises `ScannerError(Localization.Error.REMOTE_SERVER_SCAN.format(str(e).strip()), recoverable=False)` — identical to today's permanent-error raise at `remote_scanner.py:108-111`; the install path re-raises the original `SshcpError` so the UNCHANGED `_install_scanfs` except produces `ScannerError(Localization.Error.REMOTE_SERVER_INSTALL.format(str(e).strip()), recoverable=self._is_transient_ssh_error(e))` (False for "Bad hostname:") — identical to today's install-error raise at `:162-165` / `:184-187`.
...
def ssh_shell(*args):
    ...
    if self.ssh_run_command_count == 1:      # md5sum install-check (install path)
        raise SshcpError("Bad hostname: somehost")   # install-time name-resolution blip
...
|--------|----------|-----------|-------------------|-------------|
| SCAN-01 | `Bad hostname:` is classified retryable by the new matcher / retried by the shared helper (scan + install) | unit | `pytest tests/unittests/test_controller/test_scan/test_remote_scanner.py -k name_resolution -x` | ❌ Wave 0 (add to existing file) |
...
| SCAN-03 | Retry exhaustion raises `ScannerError(recoverable=False)` with the unchanged localized message (scan → REMOTE_SERVER_SCAN; install → REMOTE_SERVER_INSTALL) | unit | `pytest tests/unittests/test_controller/test_scan/test_remote_scanner.py -k "retry_exhausts or install_name_resolution_bounded" -x` | ❌ Wave 0 |
...
| Masking a real config error (security-relevant: stale state) | — | SCAN-03 ensures real permanent errors still surface after bounds on both paths; retry never silently masks. |
...
- Architecture / insertion points: HIGH — exact file:line for each integration point read and cross-checked against the debug traceback; install-path coverage (codex HIGH) verified against `scan()` → `_install_scanfs()` ordering.
