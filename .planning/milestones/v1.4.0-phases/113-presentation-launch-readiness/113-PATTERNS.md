...
- **Rate limiting** — The webhook endpoints (60/60s), the POST config-set endpoint (60/60s), the Sonarr/Radarr test-connection endpoints (5/60s), the bulk-command endpoint (10/60s), and the status endpoint (60/60s) are rate-limited. (Other mutable endpoints — e.g. single-file commands, the server-restart command, and autoqueue add/remove — are not rate-limited; do not claim a blanket "all mutable endpoints" and do not present any enumeration of un-rate-limited endpoints as complete.)
...
> - **Rate limiting (CRITICAL #1):** earlier said "mutable HTTP endpoints are rate-limited" as a blanket. WRONG — only five decorator sites carry `@rate_limit`. The POSITIVE claim must name ONLY these five (webhook, config-set, test-connection, bulk command, status) and must NOT say "all mutable endpoints." Ground-truth decorator map (enumerate these before writing the claim):
...
>   - NOT rate-limited (NON-EXHAUSTIVE — examples of un-rate-limited mutable endpoints include): single-file command endpoints (`controller.py:66-70` queue/stop/extract/delete_local/delete_remote), `POST /server/command/restart` (`server.py:16` — no `rate_limit` import), autoqueue add/remove/get (`auto_queue.py` — no `rate_limit` import at all), config-get (`config.py:24`). This NOT-list is internal guidance to the executor, NOT prose to ship — the shipped README/SECURITY prose must NOT claim a complete enumeration of un-rate-limited endpoints.
...
**Revised:** 2026-06-02 (codex adversarial-review corrections: SECURITY.md SSRF wording fixed to IP-resolution guard on *arr connection URLs; rate-limiting wording fixed to enumerate the five actual decorator sites; `POST /server/command/restart` (server.py:16) added to the NOT-rate-limited inventory and that inventory marked NON-EXHAUSTIVE so the accuracy guard cannot read as a complete enumeration; LICENSE-rename audit broadened repo-wide incl. ACKNOWLEDGMENTS.md; outbound-push honesty guard added to the fork note)
