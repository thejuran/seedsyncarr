...
- **Python `fail_under` is at `pyproject.toml:88` under `[tool.coverage.report]`**, currently `84`. Baseline measured ~85.19% (host/provisional — host-only, EXCLUDES the real-lftp integration suite). Per the baseline mandate (v1.3.0-COVERAGE-BASELINE.md lines 85-91), the authoritative "now"/"before" re-measure is the CONTAINER-INCLUSIVE run (`pytest --cov` inside the test container, which has the `lftp` binary + `sshd`, so the real-lftp suite is INCLUDED) — NOT the host-only number. The host `make coverage-python` is a provisional cross-check only, not the ratchet source.
</specifics>
